...
tgt FLASHspoiled_relax1.2s task find rewinder grads, except read ADC grads
this is the same as e05_tgtGRE_tskGREnogspoil.py, but now with more automatic restarting
and high initial learning rate
in addition now the relaxation delay is increased to 1.2s  (approx 3 x T2) to avoid any higher echoes
"""
...
#grad_moms[2:-3,:,0] = torch.ones(int(sz[0])-1).view(int(sz[0]-1),1).repeat([1,NRep]) # ADC open, readout, freq encoding
...
def init_variables():
    ...
    grad_moms_mask[1,:,0] = 1
#    grad_moms_mask[-2,:,:] = 1
    grad_moms_mask = setdevice(grad_moms_mask)
    grad_moms.zero_grad_mask = grad_moms_mask
    
    grad_moms[1,:,0] = torch.ones(1)*sz[0]*0      # remove rewinder
#    grad_moms[1,:,1] = -grad_moms[1,:,1]      # GRE/FID specific, SPOILER
    
#    grad_moms[-2,:,0] = torch.ones(1)*sz[0]      # remove spoiler gradients
...
for i in range(10):
    print('<seq> Optimization ' + str(i+1) + ' with 20 iters starts now...')
