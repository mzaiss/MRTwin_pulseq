function multiRECO_combined_abstract(single)
...
[ktraj_adc_tgt, ktraj_tgt] = seq.calculateKspace();
kmax=(max(ktraj_adc_tgt(1,1:sz(1)))-min(ktraj_adc_tgt(1,1:sz(1))))/2;
...
%  [sos_base, phase_base] = TWIXtoIMG_ADJOINT(twix_obj, scanner_dict, 1);
   [sos_base, phase_base] = TWIXtoIMG_ADJOINT_gradmoms_from_pulseq(twix_obj,ktraj_adc_tgt);
...
plot(ktraj(1,:),ktraj(2,:),'b'); hold on; % a 2D plot 
% axis('equal'); % enforce aspect ratio for the correct trajectory display
plot(ktraj_adc(1,:),ktraj_adc(2,:),'r.','MarkerSize',3); axis('square'); title(sprintf('iter %d \n k-space samples',jj))
grid on; hold off; 
set(gca,'XTick',[-kmax kmax]); set(gca,'YTick',[-kmax kmax]); set(gca,'XTickLabel',{'-k_{max}','k_{max}'}); set(gca,'YTickLabel',{'-k_{max}','k_{max}'});
axis([-kmax kmax -kmax kmax]*1.8);

  subplot(4,2,2), 
plot(ktraj_tgt(1,:),ktraj_tgt(2,:),'b'); hold on; % a 2D plot 
% axis('equal'); % enforce aspect ratio for the correct trajectory display
plot(ktraj_adc_tgt(1,:),ktraj_adc_tgt(2,:),'r.','MarkerSize',3); axis('square'); title(sprintf('target\nk-space samples'))
grid on; hold off; 
set(gca,'XTick',[-kmax kmax]); set(gca,'YTick',[-kmax kmax]); set(gca,'XTickLabel',{'-k_{max}','k_{max}'}); set(gca,'YTickLabel',{'-k_{max}','k_{max}'});
axis([-kmax kmax -kmax kmax]*1.8);

subplot(4,2,3), hold off; plot(180/pi*abs(squeeze(scanner_dict.flips(jj,1,:,1))),'r.-','MarkerSize',3,'DisplayName','flips'); title('flip angles'); ylabel('FA [°]'); axis([0 48 0 20]);
subplot(4,2,4), hold off; plot(180/pi*squeeze(scanner_dict_tgt.flips(1,:,1)),'r.-','MarkerSize',3,'DisplayName','flips tgt'); title('flip angles'); axis([0 48 0 20]);

subplot(4,2,5), imagesc(rot90((SIM_sos).',2)), title(sprintf('iter %d, rSAR %.1f',jj,SIM_SAR)), axis('image');  set(gca,'XTickLabel',[]); set(gca,'YTickLabel',[]);
subplot(4,2,6), imagesc(rot90((SIM_sos_base).',2)), title('target sim'), axis('image'); set(gca,'XTickLabel',[]); set(gca,'YTickLabel',[]);

subplot(4,2,7), imagesc(rot90(sos),[0 1]), axis('image'); set(gca,'XTickLabel',[]); set(gca,'YTickLabel',[]);
subplot(4,2,8), imagesc(rot90(sos_base),[0 1]), title('target in vivo'), axis('image'); set(gca,'XTickLabel',[]); set(gca,'YTickLabel',[]);

    set(gcf, 'Outerposition',[578    64   397   899])
    if (single==0)
