...
alpha = torch.tensor(180 * np.pi / 180)
...
# rotate grid
for i in range(NRep):
    alpha = torch.tensor(i * 180 * (1.0/(NRep-1)) * np.pi / 180)
    ...
    grad_moms[1,i,:] = (torch.matmul(rotomat,grad_moms[1,i,:].unsqueeze(1))).squeeze()
    grad_moms[2:-2,i,:] = (torch.matmul(rotomat.unsqueeze(0),grad_moms[2:-2,i,:].unsqueeze(2))).squeeze()
