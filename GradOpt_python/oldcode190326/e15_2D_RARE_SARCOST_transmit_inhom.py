...
# model transmit inhomogeneity
B1plus_inhomo = np.ones((sz[0],sz[1],1)).astype(np.float32)
B1plus_inhomo += 0.5*(np.random.rand(sz[0],sz[1],1) - 0.5)

scanner.B1plus = torch.from_numpy(B1plus_inhomo).reshape(NVox)
scanner.B1plus = scanner.setdevice(scanner.B1plus)
...
scanner.set_gradient_precession_tensor(grad_moms,refocusing=True,wrap_k=False)
