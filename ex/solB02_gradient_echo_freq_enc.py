...
# gy = make_trapezoid(channel='y', flat_area=Nread/np.sqrt(2), flat_time=20e-3, system=system)
...
# or (ii) set phantom  manually to a pixel phantom
    ...
    obj_p[20:40,10,:]=torch.tensor([1, 1, 0.1, 0.1, 0, 1]) # dimensions: PD, T1 T2, T2dash, dB0 rB1
    obj_p[20,10,:]=torch.tensor([5, 1, 0.1, 0.1, 0, 1]) # dimensions: PD, T1 T2, T2dash, dB0 rB1
    obj_p[50,10,:]=torch.tensor([5, 1, 0.1, 0.1, 0, 1]) # dimensions: PD, T1 T2, T2dash, dB0 rB1
...
major_ticks = np.arange(0, Nread*Nphase, Nread) # this adds ticks at the correct position szread
