...
plt.figure(), plt.subplot(131), plt.imshow(AA); plt.title('original')

BB=AA.copy()
BB[:-1,]=AA[1:,:] # shift manipulation 1, shift part of array

plt.subplot(132), plt.imshow(BB); plt.title('shift manipulation 1')

CC=AA.copy()
CC=np.roll(AA,-1,0) # shift manipulation 2, roll the array

plt.subplot(133), plt.imshow(CC); plt.title('shift manipulation 2 - np.roll')
...
CC=AA.clone()
CC[::2,:]=torch.flip(AA[::2,:],[0])# reverse manipulation 2, only every second line
...
plt.subplot(133), plt.imshow(CC); plt.title('shift manipulation 2 - torch.roll')
