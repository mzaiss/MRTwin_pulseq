...
we obtain something that looks exactly like a Fourier transformation from the real space to the spatial frequency space, or k-space. Actually it is an inverse Fourier transform with a positive sign in the exponent:
...
Thus, the signal for a given gradient moment g, or k, is  actually just exactly the Fourier transform of the  excited transverse magnetization. 
 The inverse Fourier transform is invertible, and the inverse of the inverse Fourier transform is the Fourier transform now with a negative sign in the exponent.
 Thus, the Fourier transform of the signal acquired at a full k-space grid will generate an image of the transverse magnetization.
...
\caption{An MR sequence scheme as picture (gradient echo,12x12 ). This is the same sequence as in Figure \ref{fig:MRsequence_GRE}. In addition to the events, also the k-space trajectory is given (bottom-right). This reflects the cumulative sum of played out gradient moments and the coverage of the acquired k-space. Full-coverage is necessary for iFFT.     } \label{fig:MRsequence_pic_GRE}
\end{figure}
...
Correctly ordered and shifted to form the complex k-space, the signal can be directly Fourier transformed using an iFFT to image domain.
...
A closer look to a two pixel gradient echo reveals that we can separate two pixels by their frequencies they send during the readout rephasing gradient. And these frequency contributions can be separated from the signal by using an iFFT.
This brought us to the general concept of iFFT reconstruction and that in MRI we measure directly points in the k-space and the k-space coordinates are determined by the gradient experienced by the transverse magnetization (compare eqns. \eqref{eq:signal_kspace} and \eqref{eq:signaltoimg}). The gradient moment used as input in the simulation is directly linked to the k-space coordinate.
\begin{itemize}
...
\end{itemize}
By these operations we can play out gradients in such a way that the k-space is equidistantly and fully covered ready for a two-dimensional iFFT. If gradients are not played out in linear order (centric reordering or EPI zig-zag) another reordering of the k-space must be performed before iFFT. Undersampled k-spaces can also be reconstructed as discussed in the radial imaging part below.
...
With the encoded signal and the iFFT reconstruction we can now generate a full GRE MRI sequence. The fully relaxed gradient echo sequence consists several repetitions of the following building block: 
\begin{itemize}
\item 90° excitation
...
\subsection{ex\_help02\_pypulseq}
...
\begin{figure}[H] 
\centering
\includegraphics[width=15cm]{img/symmetric_rf_pulse_blocks.png}
\caption{RF pulse block duration and composition} \label{fig:ex_help02_rf}
\end{figure}
...
\subsubsection{Trapezoid definition}
There are more ways of defining gradients see code and Figure \ref{fig:ex_help02_grad}.
\begin{itemize}
    \item area and duration
    \item flat\_area and flat\_time
    \item amplitude and duration
\end{itemize}
\begin{minted}[fontsize=\scriptsize, bgcolor=bg]{python}
gx1 = pp.make_trapezoid(channel='x', area=-gx.area / 2, duration=2e-3, system=system)
gx2 = pp.make_trapezoid(channel='x', flat_area=-gx.area / 2, flat_time=2e-3, system=system)
gx3 = pp.make_trapezoid(channel='x', amplitude=111, duration=2e-3, system=system)
\end{minted}

\begin{figure}[H] 
\centering
\includegraphics[width=10cm]{img/gradient_flat_dur.png}
\caption{Different ways of gradient definition} \label{fig:ex_help02_grad}
\end{figure}
...
\item  Search for the B0 inhomogeneity of the object (obj\_p.B0). In the on-resonant case this is 0. By changing the value (e.g. obj\_p.B0 +=100) you can mimic the laboratory frame dynamic. 
...
\item (Bonus) Repeat the whole sequence multiple times. Run a for loop to iFFT every repetition in the same manner.
...
\item To have full encoding you now have to Fourier transform a second time, once a loop over Nread and iFFT along Nphase, and once a loop over Nphase  and an iFFT over Nread. If you got this right, your plot will show the profiles along x of the phantom at each phase encoding position in y.
...
The idea of a faster SE is similar to the GRE EPI, we can also reuse the transverse magnetization after the first acquisition.
Thus, we start from the spin echo train of A02. We want to generate a so called turbo spin echo (TSE), or Rapid Aquisition relaxation enhanced (RARE). Thus, we need to encode the echoes of this train now.\\
\begin{enumerate}

\item Add the frequency encoding gradients. What is needed to have the same gradient echo in all repetitions? Do you need the rewinder gradient? If all echos look proper, add the phase encoding to get RARE MR imaging. Check your k-space plot carefully, as there might be some unexpected jumps. 

\item Most probably you still have imaging artifacts. As for the RARE the timing is very important, thus check again that the echo time TE/2 from center of the 90 to the center of the first 180 pulse, and the TE between the center of the 180 pulses match exactly, and that you have the ADC centered around the TE. The function calculate\_duration can help. Some events have have a rise-time or delay times that need to e considered.
...
\item As before let us decrease the recovery time. This time make it very short. You should observe an image with artifacts when going from 5 to 0.002 s. Last time we tried to get rid of higher echoes. This time we want to understand them better and use both of them.
...
We have to either iFFT all coil data, and sum it. Or we just look at one individual coil.
Either before iFFT select one coil:
\begin{minted}[fontsize=\scriptsize, bgcolor=bg]{python}
spectrum =spectrum[:,:,0]
\end{minted}
or after iFFT do:
