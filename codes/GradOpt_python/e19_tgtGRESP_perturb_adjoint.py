...
scanner.kspace_loc[2:-2,:,:] += setdevice(0.9*(torch.rand(scanner.kspace_loc[2:-2,:,:].shape) - 0.5))
#scanner.kspace_loc[2:-2,:,:] += setdevice(0.9*(torch.rand([sz[0],1,2]) - 0.5))
#scanner.kspace_loc[2:-2,:,:] += setdevice(0.9*(torch.rand([1,sz[1],2]) - 0.5))

    
# forward/adjoint pass
# %% ###
scanner.signal = signal.clone()
#scanner.discard_out_of_kspace_sig()

# generalized adjoint

B0X = torch.unsqueeze(scanner.kspace_loc[:,:,0],2) * scanner.rampX
B0Y = torch.unsqueeze(scanner.kspace_loc[:,:,1],2) * scanner.rampY

B0_grad = (B0X + B0Y).view([scanner.T,scanner.NRep,scanner.NVox])

B0_grad_adj_cos = torch.cos(B0_grad)
B0_grad_adj_sin = torch.sin(B0_grad)        

# adjoint
scanner.G_adj[:,:,:,0,0] = B0_grad_adj_cos
scanner.G_adj[:,:,:,0,1] = B0_grad_adj_sin
