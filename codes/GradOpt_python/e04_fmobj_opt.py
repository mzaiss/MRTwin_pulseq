...
class SpinSystem():
    ...
    def set_system(self):
        ...
        self.T1 = torch.ones(self.NVox, dtype=torch.float32)*1e6
        self.T2 = torch.ones(self.NVox, dtype=torch.float32)*2
        self.T2[0:NVox/2] = 0.09
        
        # set NSpins offresonance
        self.dB0 = torch.from_numpy(0*(1e-3*np.pi/180)*np.arange(0,self.NSpins).reshape([self.NSpins])).float()
...
class Scanner():
    ...
    def __init__(self,sz,NVox,NSpins,NRep,T,dt):
        ...
        self.signal = None                         # measured signal (T,NRep)
    ...
    def init_signal(self):
        signal = torch.zeros((self.T,self.NRep,4,1), dtype=torch.float32) 
        signal[:,:,3,0] = 1                                         # aux dim
    ...
    def read_signal(self,t,spins):
        if self.adc_mask[t] > 0:
            self.signal[t,:,:,0] = torch.sum(spins.M,[0,2,4]) * self.adc_mask[t]       
...
scanner = Scanner(sz,NVox,NSpins,NRep,T,dt)
scanner.get_ramps()
