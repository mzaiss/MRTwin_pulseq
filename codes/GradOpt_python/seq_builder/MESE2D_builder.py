...
class SE2D:
    ...
    def __init__(self, adc_count: int, rep_count: int, R_accel: (int,int) = (1,1), TE_list=[torch.tensor(3.8e-3)]):
        """Initialize parameters with default values."""
        self.R_accel = R_accel
        self.adc_count = adc_count
        self.event_count = adc_count + 4
        self.rep_count = rep_count // R_accel[0]
        self.pulse_dur = torch.tensor(2.0e-3)
        ...
        self.spoiler = 2*torch.full((rep_count + 1, ), float(adc_count))
    ...
    def generate_sequence(self, encoding_scheme='centric',
        ...
        for r in range(0,self.rep_count*self.R_accel[0],self.R_accel[0]):
            ...
            for n in len(self.TE_list):
                ...
                rep.event_time[1] = self.TE_list[n]
                rep.event_time[2:-2] = self.adc_time  # Readout
