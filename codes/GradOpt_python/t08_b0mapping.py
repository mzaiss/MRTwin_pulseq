...
phasemap = (phase_echo21 + phase_echo31) / (2*np.pi*dTE.item() * 2)

phasemap = phasemap.transpose([1,0])
phasemap = phasemap[::-1,::-1]

ax=plt.imshow(phasemap)
fig = plt.gcf()
fig.colorbar(ax)  
plt.title("b0 map")
plt.ion() 
...
np.save("../../data/current_b0map.npy",phasemap)


ax=plt.imshow(phase_echo21 - phase_echo31)
fig = plt.gcf()
fig.colorbar(ax)  
plt.title("phase map echo23 - echo 13")
plt.ion() 
plt.show()  
