...
experiment_id = 'p06_tgtGRESP_tskFLASH_FA_G_ET_'+str(rz)+'_lowsar'
sequence_class = "GRE"
...
#    loss_ortho = torch.sum((TT-setdevice(torch.eye(TT.shape[0])))**2) / (NVox)    
    ...
    print("loss_image: {} loss_sar {} loss_kpspace {} loss_time {}".format(loss_image, lbd_sar*loss_sar, lbd_kspace*loss_kspace, lbd_time*loss_time))
...
for i in range(9):
    opt.custom_learning_rate = [0.01,0.1,1e-2,lr_inc[i]]
