...
fullpath_seq = r"\\141.67.249.47\MRTransfer\pulseq_zero\sequences\seq200616\q23_tgtT1T2_tskT1T2prep_supervised_NN_seqNN_mag"
...
for j in range(0,extraRep):
    ...
    grad_moms[7,meas_indices[j,:] ,1] = 0
    for i in range(1,int(sz[1]/2)+1):
        grad_moms[7,j*measRepStep+i*2-1,1] = (-i)
        if i < sz[1]/2:
            grad_moms[7,j*measRepStep+i*2,1] = i
