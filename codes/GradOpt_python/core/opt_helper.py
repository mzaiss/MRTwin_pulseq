...
class OPT_helper():
    ...
    def train_model(self, training_iter = 100):
        ...
        self.aux_params = [self.use_tanh_grad_moms_cap, self.opti_mode]
        WEIGHT_DECAY = 1e-8
        
        if self.opti_mode == 'seq':
            self.optimizer = optim.Adam(self.scanner_opt_params, lr=self.learning_rate, weight_decay=WEIGHT_DECAY)
            ...
            self.optimizer = optim.Adam(list(self.NN.parameters())+self.scanner_opt_params, lr=self.learning_rate, weight_decay=WEIGHT_DECAY)
