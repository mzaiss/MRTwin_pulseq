...
NCoils = 8                                  # number of receive coil elements
...
# set coil sensitivities
B1 = torch.ones((NCoils,sz[0],sz[1],2))     # last dimension real/imag for B1 minus
B1[:,:,:,1] = 0
scanner.init_coil_sensitivities(B1)
...
B1plus = torch.zeros((scanner.NTransmitCoils,1,scanner.NVox,1,1), dtype=torch.float32)
B1plus[:,0,:,0,0] = torch.from_numpy(real_phantom_resized[:,:,4].reshape([scanner.NTransmitCoils, scanner.NVox]))
