...
#    flips[0,:,:]=flips[0,:,:]*0
    ...
    scale_param = setdevice(torch.tensor(0.1).float())
#    scale_param.zero_grad_mask = torch.ones((1,)).float()     
...
def phi_FRP_model(opt_params,aux_params):
    ...
    lbd_sar = 0.02*sz[0]**2         # switch on of SAR cost
#    loss_image = (scale_param*scanner.reco - targetSeq.target_image)
    loss_image = (scale_param*magimg_torch(scanner.reco) - magimg_torch(targetSeq.target_image))   # only magnitude optimization
