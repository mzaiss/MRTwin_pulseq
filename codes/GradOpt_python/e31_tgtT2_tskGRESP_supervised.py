...
experiment_id = 'e31_predictT2_fromGRE_2Xmeas'
sequence_class = "GRE"
...
print('e31_predictT2_fromGRE_2Xmeas')
...
# target = T2 
target = setdevice(torch.from_numpy(real_phantom_resized[:,:,2]).float())
...
for i in range(nmb_samples):
    target_db[i,:,:] = torch.from_numpy(spin_db_input[i,:,:,2:3].reshape([NVox,1]))
