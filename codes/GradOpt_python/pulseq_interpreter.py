...
pulseq = PulseqFile("pulseq_loader/tests/spiral.seq")
...
pre_pass_settings = (
    ...
    1e-4,  # Minimum magnetisation of states in pre-pass
)
...
dist = util.set_device((np.sqrt(kspace[:, 0]**2 + kspace[:, 1]**2))[:, None])
signal *= dist

reco = reconstruct(
    signal, kspace,
    resolution=size, FOV=(1, 1, 1)
)
