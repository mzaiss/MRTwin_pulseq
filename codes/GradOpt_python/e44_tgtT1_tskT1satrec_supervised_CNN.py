...
for i in range(nmb_samples):
    ...
    plt.imshow((np.abs(np.array(test_signal[2,0,:,:,0])+1j*np.array(test_signal[2,0,:,:,0])))/(np.abs(np.array(test_signal[2,1,:,:,0])+1j*np.array(test_signal[2,1,:,:,0]))))
    plt.imshow(test_target[2,:,:])
