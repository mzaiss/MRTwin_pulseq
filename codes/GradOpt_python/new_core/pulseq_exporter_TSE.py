...
def pulseq_write_EPG(seq_param, path, FOV, oversampling=1 ,plot_seq=False, dummy=False):
    ...
    if dummy:
        for i,rep in enumerate(seq_param[:5]):
            adc_start = 0
            ...
            for event in range(rep.event_count):
                ...
                if torch.abs(rep.adc_usage[event]) == 0:  
                        ...
                        if rep.pulse.usage == Seq.PulseUsage.UNDEF:
                            ...
                            if np.abs(rep.pulse.angle) > 1e-8:
                              ...
                              if nonsel:
                                  ...
                                  # gzr.amplitude=gzr.amplitude
                                  # seq.add_block(gzr)
                                  RFdur = gz_ref.rise_time + gz_ref.flat_time + gz_ref.fall_time# + gzr.rise_time + gzr.flat_time + gzr.fall_time
                    ...
                    if np.abs(gx_gradmom)>0:
                        n_samples = np.nonzero(torch.abs(rep.adc_usage)).size()[0]*oversampling
                        gx_adc_ramp = 0
                        if any(rep.adc_usage):
                            if torch.abs(rep.adc_usage[event+1]) and torch.abs(rep.gradm[event+1,0]) > 0:
                                kwargs_for_gx = {"channel": 'x', "system": system, "flat_area": rep.gradm[event+1,0].item()*deltak*n_samples, "flat_time": rep.event_time[event+1].item()*n_samples}
                                try:
                                    print()
                                    gx_adc = make_trapezoid(**kwargs_for_gx)
                                except Exception  as e:
                                    print(f'Event:{event},Rep:{i}')
                                    raise e
                    ...
                    if np.abs(gy_gradmom)>0:
                        ...
                        if any(rep.adc_usage):
                            if torch.abs(rep.adc_usage[event+1]) and torch.abs(rep.gradm[event+1,1]) > 0:   
                                kwargs_for_gy = {"channel": 'y', "system": system, "flat_area": rep.gradm[event+1,1].item()*deltak*n_samples, "flat_time": rep.event_time[event+1].item()*n_samples}
                    ...
                    if adc_start == 1:
                        ...
                        kwargs_for_adc = {"num_samples": idx_T.size()[0]*oversampling, "duration": dur, "delay":(adc_delay), "phase_offset": rf_ex.phase_offset - np.pi/4}
        ...
        for event in range(rep.event_count):
            ...
            if torch.abs(rep.adc_usage[event]) == 0:  
                ...
                if np.abs(gx_gradmom)>0:
                    ...
                    if any(rep.adc_usage):
                        if torch.abs(rep.adc_usage[event+1]) and torch.abs(rep.gradm[event+1,0]) > 0:
                            kwargs_for_gx = {"channel": 'x', "system": system, "area": rep.gradm[event+1,0].item()*deltak*n_samples, "duration": rep.event_time[event+1].item()*n_samples}
                            try:
                                print()
                                gx_adc = make_trapezoid(**kwargs_for_gx)
                            except Exception  as e:
                ...
                if np.abs(gy_gradmom)>0:
                    ...
                    if any(rep.adc_usage):
                        if torch.abs(rep.adc_usage[event+1]) and torch.abs(rep.gradm[event+1,1]) > 0:   
                            kwargs_for_gy = {"channel": 'y', "system": system, "area": rep.gradm[event+1,1].item()*deltak*n_samples, "duration": rep.event_time[event+1].item()*n_samples}
                            gy_adc = make_trapezoid(**kwargs_for_gy)
                            gy_adc_ramp = gy_adc.amplitude*gy_adc.rise_time/2                    
                    kwargs_for_gy = {"channel": 'y', "system": system, "area": gy_gradmom-gy_adc_ramp, "duration": dur}
                    try:
