...
spins = core.spins.SpinSystem_batched(sz,NVox,NSpins,batch_size,use_gpu+gpu_dev)

tgt_input = spin_db_input[:batch_size,:,:,:]
#tgt_input[0,:,:,:] = real_phantom_resized
spins.set_system(tgt_input)

#############################################################################
## Init scanner system ::: #####################################

scanner = core.scanner_batched.Scanner_fast_batched(sz,NVox,NSpins,NRep,T,NCoils,noise_std,use_gpu+gpu_dev,batch_size)
...
for i in range(nmb_samples):
    sinput = np.zeros((batch_size,sz[0],sz[1],5))
    sinput[0,:,:,:] = spin_db_input[i,:,:,:]
    spins.set_system(sinput)
    
    scanner.forward_mem(spins, event_time)
    scanner.adjoint(spins)
    
    target_db[i,:,:] = scanner.reco[0,:,:].clone()
...
def phi_FRP_model(opt_params,aux_params):
    ...
    samp_idx = np.random.choice(nmb_samples,batch_size)
    ...
    loss_image = torch.sum(loss_diff.squeeze()**2/(NVox*batch_size))
    ...
    ereco = tonumpy(scanner.reco[0,:,:].detach()).reshape([sz[0],sz[1],2])
    error = e(tonumpy(tgt[0,:,:]).ravel(),ereco.ravel())     
...
# dirty way of doing the real phantom test -- we need to refactor this mess!!
tgt_input = spin_db_input
for i in range(nmb_samples):
    tgt_input[i,:,:,:] = real_phantom_resized
spins.set_system(tgt_input[:batch_size,:,:,:])

target_db = setdevice(torch.zeros((nmb_samples,NVox,2)).float())
    
for i in range(nmb_samples):
    sinput = np.zeros((batch_size,sz[0],sz[1],5))
    sinput[0,:,:,:] = real_phantom_resized
    spins.set_system(sinput)
