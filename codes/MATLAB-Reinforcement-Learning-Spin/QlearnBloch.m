...
rewardFunc = @(x,y,z)((z-1).^2); % Reward is -(quadratic error) from upright position. Play around with different things!
...
startPt = [0,1, 0]; % Start every episode at M0

maxEpi = 2000; % Each episode is starting with the pendulum down and doing continuous actions for awhile.
maxit = 20; % Iterations are the number of actions taken in an episode.
substeps = 2; % Number of physics steps per iteration (could be 1, but more is a little better integration of the dynamics)
dt = 0.001; % Timestep of integration. Each substep lasts this long

% Torque limits -- bang-bang control
tLim = 200; %% approx 8 µT peak
actions = [-tLim:10:tLim]; % Only 3 options, Full blast one way, the other way, and off.
...
R = rewardFunc(states(:,1),states(:,2),states(:,3)); % Initialize the "cost" of a given state to be quadratic error from the goal state. Note the signs mean that -angle with +velocity is better than -angle with -velocity
Q = repmat(R,[1,length(actions)]); % Q is length(x1) x length(x2) x length(actions) - IE a bin for every action-state combination.
% Q=Q*0;
...
Vorig3 = reshape(max(Q,[],2),[length(x3),length(x2),length(x1)]);
Vorig=squeeze(Vorig3(:,:,1));
...
f2 =quiver3( 0,0,0,1,1,1); hold on;
axPend = f2.Parent;
...
plot(0.001,0,'.k','MarkerSize',5); % Pendulum axis point
view(-35,45); view(-35,45); axis([-1 1 -1 1 -1 1]); hold on;
hold off
...
subplot(1,4,[2:4]);
hold on
map3=reshape(R,[length(x3),length(x2),length(x1)]);
map = imagesc(x1,x2,map3(:,:,1));
axMap = map.Parent;
...
axMap.XLabel.String = 'x';
axMap.YLabel.String = 'y';
axMap.Visible = 'on';
...
% istead of RK i use the analytical Bloch solutions
        z2=Bloch(z1,T,dt);
        ...
        if  (z2(1)^2+z2(2)^2+ (z2(3)+1)^2) <0.01 % If we've reached upright with no velocity (within some margin), end this episode.
...
%             
            ...
            set(pathmap,'XData',sqrt(z1(1)^2+z1(2)^2));
            set(pathmap,'YData',z1(3));
%             
            % The heat map of best Q values
            V = max(Q,[],2); % Best estimated value for all actions at each state.
            fullV3 = reshape(V,[length(x3),length(x2),length(x1)]); % Make into 2D for plotting instead of a vector.
            fullV=squeeze(fullV3(:,:,1));
            set(map,'CData',fullV);
            if transpMap
...
function [Mnext]= Bloch(M, w1x,dt)
da=0;
R1=0;
THETA=atan2(w1x,da);
weff=sqrt(w1x^2+da^2)+0.00000000000000000000000000000000001;

% the back and forth transfirnation brings the angles back to the first  rotation
x_next=M(1);
y_next=M(2);
z_next=M(3);
r=sqrt(M(1)^2+M(2)^2+M(3)^2);

phi=atan2(y_next,x_next);
thet=acos(z_next/r);
% the back and forth transfirnation brings the angles back to the first  rotation

Mnext(1)=cos(THETA)*sin(THETA) + (exp(-R1*dt)*(r*cos(phi)*sin(thet) - cos(THETA)*sin(THETA))*(cos(THETA)^2*cos(dt*weff) + sin(THETA)^2)) + (2*cos(THETA)*sin(THETA)*exp(-R1*dt)*sin((dt*weff)/2)^2*(r*cos(thet) - cos(THETA)^2)) + (cos(THETA)*exp(-R1*dt)*sin(dt*weff)*r*sin(phi)*sin(thet));
Mnext(2)= exp(-R1*dt)*cos(dt*weff)*r*sin(phi)*sin(thet) - (cos(THETA)*exp(-R1*dt)*sin(dt*weff)*(r*cos(phi)*sin(thet) - cos(THETA)*sin(THETA))) + (sin(THETA)*exp(-R1*dt)*sin(dt*weff)*(r*cos(thet) - cos(THETA)^2));
Mnext(3)=cos(THETA)^2 + (exp(-R1*dt)*(r*cos(thet) - cos(THETA)^2)*(cos(THETA)^2 + sin(THETA)^2*cos(dt*weff))) + (2*cos(THETA)*sin(THETA)*exp(-R1*dt)*sin((dt*weff)/2)^2*(r*cos(phi)*sin(thet) - cos(THETA)*sin(THETA))) - (sin(THETA)*exp(-R1*dt)*sin(dt*weff)*r*sin(phi)*sin(thet));
