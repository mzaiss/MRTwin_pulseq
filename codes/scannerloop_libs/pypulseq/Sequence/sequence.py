...
class Sequence:
    ...
    def plot(self, type: str = 'Gradient', time_range=(0, np.inf), time_disp: str = 's', clear=False,signal=0):
        """
        Plot `Sequence`.
        ...
        for iB in range(1, len(self.block_events) + 1):
            ...
            if is_valid:
                ...
                if hasattr(block, 'rf'):
                    ...
                    sp13.fill_between(t_factor * (t0 + t), 0, np.angle(                        rf.signal * np.exp(1j * rf.phase_offset) * np.exp(1j * 2 * math.pi * rf.t * rf.freq_offset)),alpha=0.5)
                    sp13.fill_between(t_factor * (t0 + t), 0, np.angle(rf.signal[ic] * np.exp(1j * rf.phase_offset) * np.exp(                  1j * 2 * math.pi * rf.t[ic] * rf.freq_offset)),alpha=0.5)
                grad_channels = ['gx', 'gy', 'gz']
