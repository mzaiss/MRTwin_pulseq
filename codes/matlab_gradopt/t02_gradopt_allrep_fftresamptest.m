...
%gtruth_m = fftfull(gtruth_m); gtruth_m(1:16,:) = 0; gtruth_m(:,17:32) = 0; gtruth_m = ifftfull(gtruth_m);        % set some part of kspace to zero just for a test
...
alpha = 45*pi/180; Rmat = [cos(alpha), -sin(alpha); sin(alpha), cos(alpha)];
...
%%
kList = fftfull(gtruth_m);

v = linspace(0,sz(1)-1,sz(1));
vv = linspace(0,sz(1)*0.5,sz(1));

[Xq,Yq] = meshgrid(v);
[X,Y] = meshgrid(vv);

%rgrad_moms = -grad_moms;
rgrad_moms = permute(grad_moms,[2,1,3]);
rgrad_moms = rgrad_moms + sz(1)/2;

Vq = interp2(Xq,Yq,kList.',rgrad_moms(:,:,1),rgrad_moms(:,:,2));
Vq(isnan(Vq(:))) = 0;
...
for rep = 1:NRep
  ...
  kRecreate(:,rep) = reshape(E_all_rep{rep}, T, [])*gtruth_m(:);
end

kRecreate = kRecreate/sqrt(prod(sz));

reco = reshape(reco,sz);

figure(1),
  subplot(2,1,1), imagesc(abs(gtruth_m));
  subplot(2,1,2), imagesc(abs(reco));
