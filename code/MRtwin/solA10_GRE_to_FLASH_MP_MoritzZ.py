...
"""
experiment_id = 'solA10_GRE_to FLASH_MP05_96_MoritzZ'
...
Now you have generated the famous FLASH sequence. fast low angle shot
...
rf_event[3,:,0] = 25*np.pi/180  # 90deg excitation now for every rep
rf_event[3,:,3]=1
#rf_event[3,:,1]= torch.fmod(torch.arange(0,50*NRep,50),360)*np.pi/180 
...
event_time[1,0] =  0.5
event_time[3,:] =  2*1e-3
event_time[4,:] =  1.5*1e-3
event_time[-2,:] = 1.5*1e-3
...
gradm_event[-2,:,1] = 2.5*szread         # spoiler (even numbers sometimes give stripes, best is ~ 1.5 kspaces, for some reason 0.2 works well,too  )
