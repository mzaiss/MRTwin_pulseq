...
fullpath_seq = r"\\141.67.249.47\MRTransfer\pulseq_zero\sequences\seq200519\q30_tgtT1T2_tskT2prepT1wait_supervised_NN"
...
iteration_idx = [0,*np.arange(50,650,50)]
# iteration_idx.sort()
...
TE_preps_2 = np.abs(event_time_numpy[:,1,[i*32 for i in range(extraRep)]])
...
flips = torch.zeros((T,NRep,4), dtype=torch.float32)
flips[7,:,0] = 5*np.pi/180  # GRE/FID specific, GRE preparation part 1 : 90 degree excitation 
flips[7,:,3] = 1
# randomize RF phases
...
for i in range(0,extraRep):
    ...
    flips[7,i*measRepStep:(i+1)*measRepStep,1] = torch.tensor(scanner.phase_cycler[:(measRepStep)]).float()*np.pi/180
...
for j in range(0,extraRep):
    ...
    #  measurement
    grad_moms[8,meas_indices[j,:] ,0] = -sz[0]/2         # GRE/FID specific, rewinder in second event block
    grad_moms[8,meas_indices[j,:] ,1] = torch.linspace(-int(sz[1]/2),int(sz[1]/2-1),int(meas_indices[j,:].size))  # phase encoding blip in second event block
    grad_moms[9:-2,meas_indices[j,:] ,0] = torch.ones(int(sz[0])).view(int(sz[0]),1).repeat([1,meas_indices[j,:].size]) # ADC open, readout, freq encoding
    ...
    grad_moms[8,meas_indices[j,:] ,1] = 0
    for i in range(1,int(sz[1]/2)+1):
        grad_moms[8,j*measRepStep+i*2-1,1] = (-i)
        if i < sz[1]/2:
            grad_moms[8,j*measRepStep+i*2,1] = i
    grad_moms[-2,meas_indices[j,:] ,1] = -grad_moms[8,meas_indices[j,:] ,1]
...
for iter_idx in range(len(iteration_idx)):
    ...
    TI_wait = torch.from_numpy(TI_waits[iter_idx])
    waiting = torch.from_numpy(waitings[iter_idx])
    
    event_time[5,:] =  2e-3
    event_time[6,:] =  2e-3
    event_time[7,:] =  2*1e-3   # for 96
    event_time[-2,:] = 2*1e-3
