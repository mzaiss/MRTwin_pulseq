...
fullpath_seq = r"\\141.67.249.47\MRTransfer\pulseq_zero\sequences\seq200528\q15_tgtT2_tskT2SEPrep_supervised_seqNNvivo_full_grappa"
...
iteration_idx = [0,*np.arange(49,8050,500)]
# iteration_idx.sort()
...
TE_preps = np.abs(event_time_numpy[:,0,[i*32 for i in range(extraRep)]])*2
...
scanner = core.scanner.Scanner(sz,NVox,NSpins,Nlines,T,NCoils,noise_std,use_gpu+gpu_dev,double_precision=double_precision, coilCombineMode='adaptive', accel=R)


flips = torch.zeros((T,Nlines,2), dtype=torch.float32)
...
for i in range(0,extraRep):
    ...
    flips[1,i*measRepStep,0] = 180*np.pi/180
    flips[1,i*measRepStep,1] = 90*np.pi/180
    flips[2,i*measRepStep,0] = 90*np.pi/180
    flips[2,i*measRepStep,1] = 180*np.pi/180
...
for j in range(0,extraRep):
    ...
    grad_moms[2,j*measRepStep] = np.float(sz[0])*2.0#1e-2
    ...
    temp_mom = grad_moms[4,meas_indices[j,:] ,1]
    
    grad_moms[4,meas_indices[j,:] ,1] = temp_mom[idx]
...
for iter_idx in range(len(iteration_idx)):
    ...
    TE_prep = torch.from_numpy(TE_preps[iter_idx])
    ...
    # first action
    for i in range(0,extraRep):
        event_time[0,i*measRepStep] = TE_prep[i]/2
...
event_time = torch.from_numpy(0.08*1e-3*np.ones((T,Nlines))).float()
TE_prep = torch.tensor(np.logspace(np.log10(0.02),np.log10(1),extraRep))
# waiting = torch.from_numpy(waitings[iter_idx])
