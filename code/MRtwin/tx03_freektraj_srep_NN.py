...
#    event_time[-1,:] = 1
    ...
    gradm_event[1,:,0] = -sz[0]/2         # GRE/FID specific, rewinder in second event block
    gradm_event[1,:,1] = torch.linspace(-int(sz[1]/2),int(sz[1]/2-1),int(NRep))  # phase encoding blip in second event block
    gradm_event[2:-2,:,0] = torch.randn(int(NEvnt-4)).view(int(NEvnt-4),1).repeat([1,NRep]) * 1 # ADC open, readout, freq encoding
    gradm_event[-2,:,0] = torch.ones(1)*sz[0]*2  # GRE/FID specific, SPOILER
    gradm_event[-2,:,1] = -gradm_event[1,:,1]      # GRE/FID specific, yblip rewinder  
    gradm_event[:2,:,:] = 0
    gradm_event = setdevice(gradm_event)
...
def phi_FRP_model(opt_params,aux_params):
    ...
    adc_mask,rf_event,event_time,gradm_event = reparameterize(opt_params)
...
#    scanner.reco *= scale_param
    scanner.reco = CNN(scanner.reco.view(1,NVox,2)).view(NVox,2)
    ...
    gvec = gradm_event[1:,:,:] - gradm_event[:-1,:,:]
    gvec1 = gvec[1:,:,:]
    gvec2 = gvec[:-1,:,:]
    gvec1 = gvec1.view([gvec1.shape[0]*gvec1.shape[1],2])
    gvec2 = gvec2.view([gvec2.shape[0]*gvec2.shape[1],2])
    dot_loss = torch.sum((gvec1[:,0] * gvec2[:,0] + gvec1[:,1] * gvec2[:,1])**2)
    lbd_dot = 0
    
    loss = loss_image + lbd*loss_sar + lbd_kspace*loss_kspace + lbd_dot*dot_loss
