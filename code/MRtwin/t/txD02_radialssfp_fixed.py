...
if True:
    ...
    gradm_event[-2,:,0] = gradm_event[4,:,0]      # GRE/FID specific, SPOILER
    gradm_event[-2,:,1] = gradm_event[4,:,1]      # GRE/FID specific, SPOILER
    gradm_event = setdevice(gradm_event)
...
if 1: # NUFFT
    NCol=NRep
#    kspace=real_phantom_resized[:,:,0]+1j*0
    ...
    if 1:
        # idx=list(range(0,31))
        # idx=list(range(32))
        spectrum_resampled_x = scipy.interpolate.griddata((grid[:,:,0].ravel(), grid[:,:,1].ravel()), np.real(kspace[:,:]).ravel(), (X, Y), method='cubic',fill_value=0)
        spectrum_resampled_y = scipy.interpolate.griddata((grid[:,:,0].ravel(), grid[:,:,1].ravel()), np.imag(kspace[:,:]).ravel(), (X, Y), method='cubic',fill_value=0)
    else:
        spectrum_resampled_x = scipy.interpolate.griddata((grid[:,:,0].ravel(), grid[:,:,1].ravel()), np.real(kspace[:,:]).ravel(), (X, Y), method='cubic',fill_value=0)
        spectrum_resampled_y = scipy.interpolate.griddata((grid[:,:,0].ravel(), grid[:,:,1].ravel()), np.imag(kspace[:,:]).ravel(), (X, Y), method='cubic',fill_value=0)
...
if 0: 
    ...
    scanner.generalized_adjoint(alpha=genalpha,nmb_iter=200)
    space = scanner.reco.clone().cpu().numpy().reshape([sz[0],sz[1],2])
    space = magimg(space)
...
plt.subplot(4,6,21)
plt.imshow(np.abs(spectrum), interpolation='none'); plt.xlabel('spectrum')
plt.subplot(4,6,22)
